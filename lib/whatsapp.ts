...
export function whatsappGeneralLink(text = 'Hola! Quiero hacer una consulta sobre los iPhone disponibles.'): string {
...
export function whatsappProductLink(product: Product): string {
  const precios = [
    ...
    product.precioUsd != null ? `USD ${formatUsd(product.precioUsd).replace('$', '')}`.trim() : null,
  ]
    .filter(Boolean)
    .join(' / ')
